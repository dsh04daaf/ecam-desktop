...
const commands = {
  ...
  preview: ({ url }) => rpc(['preview', url]),
  history_list: () => rpc(['history']),
  history_clear: async () => ({ preview: true }),
  history_remove: async () => ({ preview: true }),
  open_folder: async () => { throw new Error('en la vista previa no hay explorador'); },
  restart_wrapper: async () => { throw new Error('en la vista previa no se relanza el motor de la VPS'); },
  wrapper_logs: async () => ['(sin logs en la vista previa)'],
  widevine_ready: () => rpc(['wvready']),
