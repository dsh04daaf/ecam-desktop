...
/// Lo que el descifrado de una pista necesita: seleccionar la llave y descifrar.
///
/// Existe para que `decrypt_to_file` no sepa si detrás hay un socket al wrapper
/// o Temari descifrando aquí mismo. Los dos motores tienen que comportarse
/// igual: mandar/cambiar la llave **solo cuando el URI cambia**.
pub trait KeyedDecryptor: Decryptor {
    fn ensure_key(&mut self, adam_id: &str, key_uri: &str) -> Result<()>;
}
...
impl Wrapper {
    ...
    fn ensure_key_impl(&mut self, adam_id: &str, key_uri: &str) -> Result<()> {
...
impl KeyedDecryptor for Wrapper {
    fn ensure_key(&mut self, adam_id: &str, key_uri: &str) -> Result<()> {
        self.ensure_key_impl(adam_id, key_uri)
    }
}
